...
本文档介绍了AioScrapy的核心概念和架构，帮助您理解框架的工作原理。
...
AioScrapy是一个基于异步IO的网络爬虫框架，它的架构由以下主要组件组成：
...
AioScrapy使用Python的asyncio库实现异步操作，这使得它能够高效地处理大量并发请求。
...
- **body**: Request body
- **meta**: Request metadata, can be used to pass data between requests and callbacks
- **callback**: The callback function to process the response
- **errback**: The callback function to handle request errors
...
- **meta**: Response metadata, inherited from the request's meta

## 选择器 | Selectors

AioScrapy提供了强大的选择器API，用于从HTML和XML响应中提取数据。
AioScrapy provides a powerful selector API for extracting data from HTML and XML responses.

选择器基于lxml库，支持CSS和XPath表达式：
...
详细信息请参见[下载器](downloaders.md)。
See [Downloaders](downloaders.md) for detailed information.

## 信号 | Signals

AioScrapy使用信号系统允许组件在特定事件发生时触发操作。
...
- **spider_opened**: Triggered when a spider starts
- **spider_closed**: Triggered when a spider closes
- **item_scraped**: Triggered when an item is scraped
- **request_scheduled**: Triggered when a request is scheduled
- **response_received**: Triggered when a response is received

## 设置 | Settings

AioScrapy提供了丰富的配置选项，允许您自定义爬虫的行为。
AioScrapy provides a rich set of configuration options that allow you to customize the behavior of your spiders.

详细信息请参见[配置参考](settings.md)。
