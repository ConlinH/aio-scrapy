    ...
    def scrape_response(self, scrape_func, response, request, spider):
        ...
        def process_spider_input(response):
            for method in self.methods['process_spider_input']:
                try:
                    ...
                    iterable_or_exception = scrape_func(exception, request, spider)
        ...
        def _evaluate_iterable(iterable, exception_processor_index, recover_to):
            try:
                ...
                exception_result = process_spider_exception(ex, exception_processor_index)
                if isinstance(exception_result, (Exception, BaseException)):
                    raise
                recover_to.extend(exception_result)

        def process_spider_exception(exception, start_index=0):
        ...
        def process_spider_output(result, start_index=0):
            ...
            for method_index, method in enumerate(method_list, start=start_index):
                ...
                try:
                    ...
                    exception_result = process_spider_exception(ex, method_index + 1)
                    if isinstance(exception_result, (Exception, BaseException)):
                        raise
        ...
        try:
            iterable = process_spider_input(response)
        except (Exception, BaseException) as exc:
            result = process_spider_exception(exc)
        else:
            result = process_callback_output(iterable)
        return result
